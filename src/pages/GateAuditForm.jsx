...
// Use a namespace import so we can fall back if createTruckGateAudit isn't exported yet.
import * as audits from "../lib/audits";
...
const LABEL = {
  ...
  /* Inbound — Inspection & Verification */
...
export default function GateAuditForm() {
  // make header full-bleed on this page
  useEffect(() => {
    const headerEl =
  ...
  /* Resolve supervisor (pull from guards table) */
  useEffect(() => {
    (async () => {
  ...
  /* Guards roster */
  useEffect(() => {
    (async () => setRoster((await audits.fetchGuards()) || []))();
  }, []);
  ...
  async function submitAudit() {
    setSaving(true);
    try {
      // Prefer separate truck gate table; fall back to interior if not available.
      const saveFn =
        audits.createTruckGateAudit &&
        typeof audits.createTruckGateAudit === "function"
          ? audits.createTruckGateAudit
          : audits.createAudit;
      ...
      await saveFn({
        ...
        gate_type: gateType,
        post: gateType, // keep non-null for fallback to interior table
        shift,
        guard_id: guardId,
