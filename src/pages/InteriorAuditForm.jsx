...
/* ----------------------------- Catalog ------------------------------ */
const POSTS = ["B400", "CCTV/Dispatch", "Main Lobby", "Greenstone", "VMF"];
...
export default function InteriorAuditForm() {
  ...
  /* ✅ Gate: only need a Guard to submit */
  ...
  async function submitAudit() {
    ...
    try {
      await createAudit({
        supervisor: supervisorDisplay, // may be empty string if not mapped
        post,
        shift,
        guard_id: guardId,
  ...
  return (
    <div className="py-5 iaf-page">
      <style>{`
        ...
        /* Revert: don't hide "Open Sandbox" — just make it non-blocking */
        [aria-label*="Open Sandbox"],
        [data-testid*="open-in-sandbox"],
        a[href*="codesandbox"],
        button[title*="Open Sandbox"] {
          pointer-events: none !important;
          opacity: 0.65; /* visible but not interactive */
        }
      `}</style>
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="flex justify-end pt-3 pb-28 md:pb-6">
              <button
                ...
                title={canSubmit ? "Submit Audit" : "Choose a Guard first"}
              >
