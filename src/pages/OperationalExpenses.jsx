// src/pages/OperationalExpenses.jsx
...
/** Build value for <input type="datetime-local"> as YYYY-MM-DDTHH:mm in ET. */
...
/** Convert a local ET ISO (YYYY-MM-DDTHH:mm) to UTC ISO for DB filters/inserts. */
function toZonedUTCISO(localIso) {
  ...
  // figure the ET offset for that instant (handles DST)
...
/* ===================== Centered Modals ===================== */
function ConfirmModal({ open, onCancel, onConfirm, data }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[90]">
      <div className="absolute inset-0 bg-black/35" onClick={onCancel} />
      <div className="relative h-full w-full flex items-center justify-center p-4">
        <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-[#0f1215] shadow-2xl max-w-md w-full">
          <div className="px-5 py-4 border-b border-black/5 dark:border-white/10">
            <div className="text-base font-semibold">Confirm &amp; Send</div>
            <div className="text-sm text-slate-600 dark:text-slate-300/80 mt-1">
              Please confirm the details below before submitting this expense.
            </div>
          </div>
          <div className="px-5 py-4 text-sm">
            <div className="space-y-1.5">
              <div>
                <strong>Date &amp; Time (ET):</strong> {data.whenLabel}
              </div>
              <div>
              ...
              <div>
                <strong>Amount:</strong> {currency(data.amountNum || 0)}
              </div>
              {data.receiptName ? (
                <div>
                  <strong>Receipt:</strong> {data.receiptName}
                </div>
              ) : null}
              {data.desc ? (
                <div>
          ...
          <div className="px-5 py-3 border-t border-black/5 dark:border-white/10 flex justify-end gap-2">
            ...
            <button className="btn-pill btn-green" onClick={onConfirm}>
              Confirm &amp; Send
            </button>
          </div>
        </div>
...
/* ===================== Page ===================== */
const BUCKET_NAME = "finances"; // single private bucket per your SQL
...
export default function OperationalExpenses() {
  ...
  const [sentPopup, setSentPopup] = useState("");
  ...
  async function performSave() {
    ...
    try {
      ...
      const path = `ops/${Date.now()}_${safe(receipt.name)}`;
      ...
      // success popup + refresh
      setSentPopup("Your expense has been recorded.");
      refresh();
    } catch (e) {
      console.error(e);
      // keep validation style consistent—use a lightweight toast/alert?
  ...
  async function refresh() {
    ...
    const { data, error } = await supabase
      .from("operational_expense_logs")
      .select(
        "id, occurred_at, category, vendor, amount_usd, description, receipt_path"
      )
  ...
  // helpers for confirm sheet
  const confirmData = {
    whenLabel: (() => {
      try {
        return formatET(toZonedUTCISO(whenISO));
      } catch {
        return whenISO;
      }
    })(),
    category,
    vendor,
    amountNum: Number(String(amount).replace(/[$,\s]/g, "") || 0),
  ...
  return (
    <div className="py-8">
      <style>{`
        ...
        /* Modal & pill buttons (same vibe as your other pages) */
      ...
      <div className="page-full px-4 md:px-6 bg-gradient-to-b from-[#fafbfc] via-[#f7f6f3] to-[#f6f5f2] dark:from-[#2a3040] dark:via-[#262c38] dark:to-[#232835]">
        <header className="mb-5">
          ...
          <p className="text-sdg-slate dark:text-white/70 mt-1">
            Track purchases made by managers for operations.
          </p>
        </header>

        <Frame title="Add Operational Expense">
        ...
        <div className="mt-6">
          <Frame title="Operational Expense Logs (Weekly)">
      ...
      {/* Success popup */}
      <SentPopup message={sentPopup} onClose={() => setSentPopup("")} />
    </div>
  );
}
